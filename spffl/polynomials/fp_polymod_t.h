...
#ifndef SPFFL_POLYNOMIALS_FP_POLYMOD_T_H
#define SPFFL_POLYNOMIALS_FP_POLYMOD_T_H
...
class fp_polymod_t {
  ...
  fp_polymod_t(const spffl::intmath::intmod_t &c, const fp_poly_t &modulus)
      : fp_polymod_t(fp_poly_t(c), modulus) {}
  ...
  fp_polymod_t operator*(spffl::intmath::intmod_t a) const {
    return fp_polymod_t(residue_ * a, modulus_);
  }
  ...
  fp_polymod_t operator*(int a) const {
    return *this * spffl::intmath::intmod_t(a, residue_.get_characteristic());
  }
  fp_polymod_t &operator*=(int a) {
    return *this *= spffl::intmath::intmod_t(a, residue_.get_characteristic());
  }

  bool operator>(const fp_polymod_t &that) const { return that < *this; }
  bool operator<=(const fp_polymod_t &that) const { return !(that < *this); }
  bool operator>=(const fp_polymod_t &that) const { return !(*this < that); }

  /// Remainder in the ring (for CLI/compat). In a field, zero when divisor is unit.
  fp_polymod_t operator%(const fp_polymod_t &that) const {
    check_moduli(that);
    if (that.residue_.is_zero()) {
      throw std::runtime_error("fp_polymod_t::operator%: division by zero");
    }
    fp_poly_t q, r;
    residue_.quot_and_rem(that.residue_, q, r);
    // r may have degree >= deg(modulus_), reduce mod modulus_
    fp_poly_t q2, r2;
    r.quot_and_rem(modulus_, q2, r2);
    return fp_polymod_t(r2, modulus_);
  }
  fp_polymod_t &operator%=(const fp_polymod_t &that) {
    *this = *this % that;
    return *this;
  ...
  /// Legacy compat: characteristic of the field.
  int get_characteristic() const { return residue_.get_characteristic(); }

  /// Legacy compat: 0 or 1 in this field (instance method).
  fp_polymod_t prime_subfield_element(int v) const {
    return fp_polymod_t(residue_.prime_subfield_element(v), modulus_);
  }

  /// Legacy compat: 0 or 1 in the field (static).
