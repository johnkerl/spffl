...
inline std::optional<f2n_poly_t> f2n_poly_from_string(const std::string &s, const f2_poly_t &m) {
  int num_colons = 0;
  for (char c : s) {
    if (c == ':')
      ++num_colons;
  ...
  int argc = spffl::base::tokenize(buf.data(), ":", argv.data(), static_cast<int>(argv.size()));
  if (argc < 1)
    return std::nullopt;
...
inline std::optional<fpn_poly_t> fpn_poly_from_string(const std::string &s, const fp_poly_t &m) {
  int num_colons = 0;
  for (char c : s) {
    if (c == ':')
      ++num_colons;
