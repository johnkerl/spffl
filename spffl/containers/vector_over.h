...
class vector_over {
  ...
  template <typename It> vector_over(It first, It last) : data_(first, last) {}
  ...
  int get_num_elements() const { return static_cast<int>(data_.size()); }

  void resize(int n) {
    if (n < 0)
      throw std::invalid_argument("vector_over::resize: negative size");
  ...
  bool operator!=(const vector_over &that) const { return !(*this == that); }
  ...
  /// Parse "[e1 e2 ...]" (whitespace-delimited). *this must already have one element (for
  /// modulus/context).
  ...
  /// Read from file or stdin ("-" / "@"). *this must already have one element for parameterized
  /// types.
