...
// ----------------------------------------------------------------
fp_polyrat_t fp_polyrat_t::prime_subfield_element(int v) const {
  spffl::polynomials::fp_poly_t a = this->numer.prime_subfield_element(v);
  return fp_polyrat_t(a);
}

// ----------------------------------------------------------------
// This is a static method.
fp_polyrat_t fp_polyrat_t::prime_subfield_element(int v, int p) {
  return fp_polyrat_t(spffl::polynomials::fp_poly_t(spffl::intmath::intmod_t(v, p)));
}

// ----------------------------------------------------------------
int fp_polyrat_t::get_characteristic(void) const { return this->numer.get_characteristic(); }
...
fp_polyrat_t fp_polyrat_t::operator%(const fp_polyrat_t &that) {
  ...
  if (that == zero) {
    std::stringstream ss;
    ss << "fp_polyrat_t: Divide by zero.\n";
...
fp_polyrat_t fp_polyrat_t::exp(int e) const {
  ...
  fp_polyrat_t zero(this->numer.prime_subfield_element(0));
  fp_polyrat_t one(this->numer.prime_subfield_element(1));
  fp_polyrat_t rv = one;

  if (e == 0) {
    if (*this == zero) {
      std::stringstream ss;
      ss << "fp_polyrat_t::exp:  0 ^ 0 undefined.\n";
      throw spffl::exception_t(ss.str());
    }
    return one;
  } else if (e < 0) {
    if (*this == zero) {
      std::stringstream ss;
      ss << "fp_polyrat_t::exp:  division by zero.\n";
      throw spffl::exception_t(ss.str());
    }
    if (e == -e) {
      std::stringstream ss;
      ss << "fp_polyrat_t::exp:  can't handle "
            "MIN_FPPOLY.\n";
      throw spffl::exception_t(ss.str());
    }
    xp = one / xp;
...
// ----------------------------------------------------------------
bool fp_polyrat_t::operator!=(const fp_polyrat_t &that) const { return !(*this == that); }
...
// ----------------------------------------------------------------
bool fp_polyrat_t::operator!=(spffl::polynomials::fp_poly_t that) const { return !(*this == that); }
