...
TEST_CASE("Ring_element: double satisfies (numeric ring)") { static_assert(Ring_element<double>); }
...
TEST_CASE(
  "Euclidean_domain: int has quot_rem and gcd in spirit (member form not required for int)") {
...
TEST_CASE("Ring_element_with_io: int has stream ops") { static_assert(Ring_element_with_io<int>); }
...
TEST_CASE("has_get_characteristic_v helper") {
  struct WithChar {
    int get_characteristic() const { return 2; }
  };
  struct WithoutChar {};
...
template <typename E> E compute_gcd_then_remainder(E const &a, E const &b, E &q, E &r) {
