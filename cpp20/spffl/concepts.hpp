...
template <typename T>
concept Ring_element_with_io = Ring_element<T> && requires(T const& a, std::ostream& os, std::istream& is) {
...
template <typename E>
concept Euclidean_domain = Ring_element<E> && requires(
    E const& a, E const& b, E& q, E& r) {
...
template <typename E>
concept Euclidean_domain_with_ext_gcd = Euclidean_domain<E> && requires(
    E const& a, E const& b, E& m, E& n) {
...
template <typename P>
concept Polynomial_ring_element = Euclidean_domain<P> && requires(
    P const& p, P const& q, P& qout, P& rout, int n) {
...
template <typename P>
concept Polynomial_with_ext_gcd = Polynomial_ring_element<P> && requires(
    P const& a, P const& b, P& m, P& n) {
...
concept Matrix_over = Ring_element<T> && requires(
    M const& A, M const& B, std::size_t i) {
...
concept Matrix_vector_product = Matrix_over<M, T> && Vector_over<V, T> && requires(M const& A, V const& x) {
...
struct has_get_characteristic : std::false_type {};

template <typename T>
struct has_get_characteristic<T, std::void_t<decltype(std::declval<T const&>().get_characteristic())>>
    : std::true_type {};
