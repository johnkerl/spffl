...
template <typename E>
  requires std::same_as<E, int> ||
           (spffl::concepts::Euclidean_domain<E> &&
...
public:
  ...
  residue_of operator-() const {
    return residue_of(negate(residue_), modulus_);
  }
...
private:
  ...
  void reduce() {
    ...
    if constexpr (std::is_same_v<E, int>) {
      ...
      if (mod_abs != 0 && residue_ < 0) {
        ...
        if (residue_ < 0) residue_ += mod_abs;
      }
    }
  }
  ...
  bool recip_polynomial(residue_of& rinv) const {
    ...
    if (!g0.recip(g0_inv)) return false;
    E minv = m * g0_inv;
    E q, r;
