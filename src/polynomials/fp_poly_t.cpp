...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c0) {
  this->degree = 0;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[0] = c0;
...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c1, intmod_t c0) {
  this->degree = 1;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[1] = c1;
  this->coeffs[0] = c0;
...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c2, intmod_t c1, intmod_t c0) {
  this->degree = 2;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[2] = c2;
  this->coeffs[1] = c1;
...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c3, intmod_t c2, intmod_t c1, intmod_t c0) {
  this->degree = 3;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[3] = c3;
  this->coeffs[2] = c2;
...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c4, intmod_t c3, intmod_t c2, intmod_t c1,
                   intmod_t c0) {
  this->degree = 4;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[4] = c4;
...
// ----------------------------------------------------------------
fp_poly_t::fp_poly_t(intmod_t c5, intmod_t c4, intmod_t c3, intmod_t c2,
                   intmod_t c1, intmod_t c0) {
  this->degree = 5;
  this->coeffs = new intmod_t[this->degree + 1];
  this->coeffs[5] = c5;
  this->coeffs[4] = c4;
...
// ----------------------------------------------------------------
fp_poly_t fp_poly_t::operator+(intmod_t a) const {
  fp_poly_t rv(*this);
  rv.coeffs[0] += a;
  rv.recompute_degree();
  return rv;
...
// ----------------------------------------------------------------
fp_poly_t fp_poly_t::operator*(intmod_t a) {
  fp_poly_t rv = *this;
  for (int i = 0; i <= rv.degree; i++)
    rv.coeffs[i] *= a;
  rv.recompute_degree();
...
// ----------------------------------------------------------------
void fp_poly_t::quot_and_rem(fp_poly_t &that, fp_poly_t &rquot, fp_poly_t &rrem) {
  intmod_t zero = this->coeffs[0] - this->coeffs[0];
...
// ----------------------------------------------------------------
fp_poly_t gcd(fp_poly_t a, fp_poly_t b) { return a.gcd(b); }
...
// ----------------------------------------------------------------
intmod_t fp_poly_t::eval(intmod_t c) {
...
// ----------------------------------------------------------------
intmod_t fp_poly_t::get_coeff(int deg) const {
  if (deg < 0)
...
// ----------------------------------------------------------------
void fp_poly_t::set_coeff(int pos, intmod_t c) {
  if (pos < 0)
...
// ----------------------------------------------------------------
int fp_poly_t::operator==(int v) const {
  int d = this->degree;
  if (d != 0)
    return 0;
...
// ----------------------------------------------------------------
int fp_poly_t::operator!=(int v) const { return !(*this == v); }

// ----------------------------------------------------------------
int fp_poly_t::operator==(fp_poly_t that) const {
...
// ----------------------------------------------------------------
int fp_poly_t::operator<(fp_poly_t that) const { return this->cmp(CMP_LT, that); }

// ----------------------------------------------------------------
int fp_poly_t::operator>(fp_poly_t that) const { return this->cmp(CMP_GT, that); }
...
// ----------------------------------------------------------------
int fp_poly_t::cmp(int cmp, fp_poly_t &that) const {
...
// ----------------------------------------------------------------
void fp_poly_t::promote_and_add(intmod_t c0) {
  this->promote();
  this->coeffs[0] = c0;
}
