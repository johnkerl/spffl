...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t::f2poly_t(int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
  this->parts[0] = c0 & 1;
}
f2poly_t::f2poly_t(int c1, int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
  this->parts[0] = ((c1 & 1) << 1) | (c0 & 1);
}
f2poly_t::f2poly_t(int c2, int c1, int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
  this->parts[0] = ((c2 & 1) << 2) | ((c1 & 1) << 1) | (c0 & 1);
}
f2poly_t::f2poly_t(int c3, int c2, int c1, int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
  this->parts[0] =
      ((c3 & 1) << 3) | ((c2 & 1) << 2) | ((c1 & 1) << 1) | (c0 & 1);
}
f2poly_t::f2poly_t(int c4, int c3, int c2, int c1, int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
  this->parts[0] = ((c4 & 1) << 4) | ((c3 & 1) << 3) | ((c2 & 1) << 2) |
                   ((c1 & 1) << 1) | (c0 & 1);
}
f2poly_t::f2poly_t(int c5, int c4, int c3, int c2, int c1, int c0) {
  this->num_parts = 1;
  this->parts = new unsigned[1];
...
#ifndef F2POLY_SMALL
f2poly_t f2poly_t::from_base_rep(unsigned b) {
  f2poly_t rv;
  rv.num_parts = 1;
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::from_string(char *string) {
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t::f2poly_t(const f2poly_t &that) {
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t f2poly_t::prime_sfld_elt(int v) const { return f2poly_t(v & 1); }
#endif

// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::get_char(void) { return 2; }
#endif

// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t &f2poly_t::operator=(f2poly_t that) {
  if (this->parts)
    delete[] this->parts;
  this->num_parts = that.num_parts;
  this->parts = new unsigned[this->num_parts];
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t &f2poly_t::operator=(int bit) {
  if (this->parts)
    delete[] this->parts;
  this->num_parts = 1;
  this->parts = new unsigned[this->num_parts];
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
f2poly_t f2poly_t::operator+(f2poly_t that) const {
  int i;
  f2poly_t rv;
...
// ----------------------------------------------------------------
f2poly_t f2poly_t::operator-(f2poly_t that) const { return *this + that; }

// ----------------------------------------------------------------
f2poly_t f2poly_t::operator-(void) const { return *this; }
...
f2poly_t f2poly_t::operator*(f2poly_t that) const {
  ...
  if (prod_deg > F2POLY_MAX_DEGREE) {
    std::cerr << "f2poly_t: multiplication overflow.\n";
    exit(1);
  }
...
void f2poly_t::quot_and_rem(f2poly_t &that, f2poly_t &rquot, f2poly_t &rrem) {
  int dividend_l1_pos;
  int divisor_l1_pos;
  int l1_diff;
  f2poly_t shift_divisor;
...
void f2poly_t::quot_and_rem(f2poly_t &that, f2poly_t &rquot, f2poly_t &rrem) {
  ...
  if (that == 0) {
    // Divisor is zero.
    std::cerr << "f2poly_t::quot_and_rem:  Divide by zero.\n";
    exit(1);
  }
  divisor_l1_pos = that.find_degree();
...
// ----------------------------------------------------------------
f2poly_t f2poly_t::gcd(f2poly_t &that) {
  f2poly_t c, q, r;
...
// ----------------------------------------------------------------
f2poly_t gcd(f2poly_t a, f2poly_t b) { return a.gcd(b); }
...
f2poly_t f2poly_t::exp(int e) {
  ...
  if (*this == zero) {
    if (e == 0) {
      std::cerr << "f2poly_t::exp:  0 ^ 0 undefined.\n";
      exit(1);
    } else if (e < 0) {
      std::cerr << "f2poly_t::exp:  division by zero.\n";
      exit(1);
    } else {
      return zero;
    }
    ...
    // Degree 1 or higher.
    if (e < 0) {
      std::cerr << "f2poly_t::exp:  division by non-unit.\n";
      exit(1);
    } else {
      while (e != 0) {
        if (e & 1)
...
// ----------------------------------------------------------------
bit_t f2poly_t::eval(bit_t c) { return bit_t(this->eval(c.get_residue())); }
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::zcount_one_bits(void) {
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::find_degree(void) const {
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::operator==(int v) const {
  for (int i = this->num_parts - 1; i > 0; i--)
    if (this->parts[i] != 0)
      return 0;
...
int f2poly_t::operator!=(int v) const { return !(*this == v); }

int f2poly_t::operator==(f2poly_t that) const {
...
int f2poly_t::operator<(f2poly_t that) const { return this->cmp(CMP_LT, that); }

int f2poly_t::operator>(f2poly_t that) const { return this->cmp(CMP_GT, that); }
...
int f2poly_t::cmp(int op, f2poly_t &that) const {
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
std::ostream &operator<<(std::ostream &os, const f2poly_t &poly) {
  if (poly.num_parts <= 0) {
    std::cerr << "f2poly_t ostream <<:  empty polynomial.\n";
    exit(1);
  }
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
void f2poly_t::debug_print(std::ostream &os) {
  std::ios_base::fmtflags svflags = os.flags();
  os.flags(std::ios_base::hex);
...
#ifdef F2POLY_SMALL
void f2poly_t::debug_print(std::ostream &os) { os << *this; }
#endif

// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
std::istream &operator>>(std::istream &is, f2poly_t &poly) {
  int c;
  poly = f2poly_t(0);
  int num_nybbles = 0;
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
std::istringstream &operator>>(std::istringstream &iss, f2poly_t &poly) {
  int c;
  poly = f2poly_t(0);
  int num_nybbles = 0;
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
void f2poly_t::promote_n(unsigned shamt) {
  // For comparison:
...
// ----------------------------------------------------------------
#ifndef F2POLY_SMALL
int f2poly_t::bit_at(int pos) const {
...
#ifndef F2POLY_SMALL
void f2poly_t::dprint(std::ostream &os, int reqdeg) const {
...
// ----------------------------------------------------------------
#ifdef F2POLY_SMALL
void f2poly_t::dprint(std::ostream &os, int deg) const {
...
#ifndef F2POLY_SMALL
void f2poly_t::set_coeff(int pos, bit_t b) {
  int which_part = pos >> F2POLY_PART_LOG;
  int which_bit = pos & F2POLY_PART_MASK;
...
#ifndef F2POLY_SMALL
void f2poly_t::extend_parts(int new_num_parts) {
  int i;
...
void f2poly_t::trim_parts(void) {
  int d = this->find_degree();
  this->num_parts = (d >> F2POLY_PART_LOG) + 1;
}

void f2poly_t::check_neg_pos(int pos) const {
  if (pos < 0) {
...
// ----------------------------------------------------------------
#ifdef F2POLY_SMALL
void f2poly_t::bounds_check_abend(int deg) const {
  if ((deg < 0) || (deg > (int)F2POLY_MAX_DEGREE)) {
    std::cerr << "f2poly_t: degree " << deg
