...
public:
  ...
  void quot_and_rem(f2poly_t &that, f2poly_t &rquot, f2poly_t &rrem);
  ...
  friend std::ostream &operator<<(std::ostream &os, const f2poly_t &poly);
  friend std::istream &operator>>(std::istream &is, f2poly_t &poly);
...
// ================================================================
#ifdef F2POLY_SMALL
inline f2poly_t::f2poly_t(void) { this->bits = 0; }

inline f2poly_t::f2poly_t(int c0) { this->bits = (c0 & 1); }
...
inline f2poly_t::f2poly_t(const f2poly_t &that) { this->bits = that.bits; }
...
inline int f2poly_t::get_char(void) { return 2; }
...
inline int f2poly_t::find_degree(void) const { return find_msb_32(this->bits); }

inline int f2poly_t::operator==(int v) const {
  return this->bits == (unsigned)(v & 1);
}

inline int f2poly_t::operator!=(int v) const { return !(*this == v); }
...
inline void f2poly_t::increment(void) { this->bits++; }
