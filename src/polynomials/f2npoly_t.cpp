...
f2npoly_t::f2npoly_t(int c5, int c4, int c3, int c2, int c1, int c0,
                     f2poly_t m) {
  this->degree = 5;
  this->coeffs = new f2polymod_t[this->degree + 1];
...
// ----------------------------------------------------------------
int f2npoly_t::get_char(void) { return 2; }
...
// ----------------------------------------------------------------
void f2npoly_t::quot_and_rem(f2npoly_t &that, f2npoly_t &rquot,
                             f2npoly_t &rrem) {
  f2polymod_t zero = this->coeffs[0] - this->coeffs[0];
...
// ----------------------------------------------------------------
f2npoly_t gcd(f2npoly_t a, f2npoly_t b) { return a.gcd(b); }
...
// ----------------------------------------------------------------
int f2npoly_t::find_degree(void) const { return this->degree; }
...
// ----------------------------------------------------------------
int f2npoly_t::operator!=(int v) const { return !(*this == v); }
...
// ----------------------------------------------------------------
std::ostream &operator<<(std::ostream &os, const f2npoly_t &poly) {
...
// ----------------------------------------------------------------
std::istream &operator>>(std::istream &is, f2npoly_t &poly) {
  f2poly_t r;
  ...
  if (m <= 10) {
    ...
    if (!isdigit(c)) {
      std::cerr << "f2npoly_t: non-numeric input '" << c << "'\n";
      return is;
    }
    r = c - '0';
...
std::istringstream &operator>>(std::istringstream &iss, f2npoly_t &poly) {
  f2poly_t r;
  f2poly_t m = poly.coeffs[0].get_modulus();
  poly = f2npoly_t(f2polymod_t(0, m));
  ...
  iss >> r;
  if (iss.fail()) {
    std::cerr << "f2npoly_t: scan failure at \"" << iss.str() << "\"\n";
    return iss;
  }
  poly.coeffs[0] = f2polymod_t(r, m);
