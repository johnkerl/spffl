...
// ----------------------------------------------------------------
int fppolymod_t::get_char(void) { return this->residue.get_char(); }
...
fppolymod_t fppolymod_t::operator/(fppolymod_t that) {
  ...
  fppolymod_t bi;
  if (!that.recip(bi)) {
    std::cerr << "fppolymod_t::operator/:  zero or zero divisor: "
...
int fppolymod_t::recip(fppolymod_t &rinv) {
  ...
  if (!g.get_coeff(0).recip(c0i)) {
    std::cerr << "fppolymod_t::recip:  zero or zero divisor "
              << " in GCD " << g << ".\n";
    exit(1);
  }
  a *= c0i;
...
// ----------------------------------------------------------------
std::istringstream &operator>>(std::istringstream &iss, fppolymod_t &a) {
...
// ----------------------------------------------------------------
int fppolymod_t::operator!=(fppolymod_t that) const { return !(*this == that); }

// ----------------------------------------------------------------
int fppolymod_t::operator==(int that) const { return this->residue == that; }
...
// ----------------------------------------------------------------
fppoly_t fppolymod_t::get_residue(void) const { return this->residue; }

// ----------------------------------------------------------------
fppoly_t fppolymod_t::get_modulus(void) const { return this->modulus; }

// ----------------------------------------------------------------
void fppolymod_t::check_moduli(fppolymod_t &that) const {
  if (this->modulus != that.modulus) {
    std::cerr << "fppolymod_t: mixed moduli " << this->modulus << ", "
              << that.modulus << ".";
    std::cerr << std::endl;
    exit(1);
  }
