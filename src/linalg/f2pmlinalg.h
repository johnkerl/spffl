...
f2npoly_t f2pm_char_poly(tmatrix<f2polymod_t> &A);

tmatrix<f2polymod_t> f2np_companion_matrix(f2npoly_t chpol);
...
int f2pm_matrix_is_dable(tmatrix<f2polymod_t> &A, f2poly_t &rsplitter_modulus,
                         tvector<f2polymod_t> &reigenvalues);

tvector<f2polymod_t> ft_vector_from_base_rep(int base_rep, f2poly_t m, int len);
