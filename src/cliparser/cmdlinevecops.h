...
template <class element_type>
static void E(
	vec_lex_ctx_t<element_type> & rlex_ctx,
	tstack<vecatom_t<element_type> > & rstack);

// ----------------------------------------------------------------
template <class element_type>
static const char * vec_token_desc(int t)
{
	switch (t) {
	case L_VEC:       return "vector";    break;
	...
	if      (strcmp(s, "+")         == 0) rlex_ctx.token = L_PLUS;
		...
		if (rlex_ctx.in_pow_rhs) {
			...
			if (strchr(rlex_ctx.argv[rlex_ctx.argi], '[')) {
				if (!rlex_ctx.atom.vec_val.bracket_in(
	...
	tstack<vecatom_t<element_type> > & rstack)
{
	...
	switch (rlex_ctx.token) {
	case L_UMINUS: C.vec_val = -A.vec_val; break;
	case L_PLUS:   C.vec_val = A.vec_val + B.vec_val; break;
	case L_MINUS:  C.vec_val = A.vec_val - B.vec_val; break;
	case L_MUL:    C.vec_val = A.vec_val * B.vec_val; break;

	case L_EQ: C.int_val = A.vec_val == B.vec_val; break;
	case L_NE: C.int_val = A.vec_val != B.vec_val; break;

	case L_EXP: C.vec_val = A.vec_val.exp(B.int_val); break;
	...
	int expected_token)
{
	if (rlex_ctx.token == expected_token) {
		...
		std::cerr << "Expected "
			<< vec_token_desc<element_type>(expected_token)
			<< "; got "
			<< vec_token_desc<element_type>(rlex_ctx.token)
			<< ".\n";
		exit(1);
