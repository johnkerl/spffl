...
template <class element_type>
static void E(
	lex_ctx_t<element_type> & rlex_ctx,
	tstack<atom_t<element_type> > & rstack);

// ----------------------------------------------------------------
template <class element_type>
static const char * token_desc(int t)
{
	switch (t) {
	...
	tstack<atom_t<element_type> > & rstack)
{
	...
	case L_EXP:
		c.type_val = clo_exp(a.type_val, b.int_val,
			rlex_ctx.zero, rlex_ctx.one);
		break;
	...
	int expected_token)
{
	if (rlex_ctx.token == expected_token) {
		...
		std::cerr << "Expected "
			<< token_desc<element_type>(expected_token)
			<< "; got "
			<< token_desc<element_type>(rlex_ctx.token)
			<< ".\n";
		exit(1);
