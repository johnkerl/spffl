...
template <class element_type> struct ed_matatom_t {
...
template <class element_type> struct ed_mat_lex_ctx_t {
  int argi;
  int argc;
  char **argv;
...
static void ed_E(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack);

// ----------------------------------------------------------------
template <class element_type> static const char *ed_mat_token_desc(int t) {
  switch (t) {
  case ED_L_MAT:
    return "matrix";
    break;
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_matlexinit(ed_mat_lex_ctx_t<element_type> &rlex_ctx, int argc,
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_matlexan(ed_mat_lex_ctx_t<element_type> &rlex_ctx) {
  ...
  if (strcmp(s, "+") == 0)
    rlex_ctx.token = ED_L_PLUS;
  else if (strcmp(s, "-") == 0)
    rlex_ctx.token = ED_L_MINUS;
  else if (strcmp(s, "*") == 0)
    rlex_ctx.token = ED_L_MUL;
  else if (strcmp(s, ".") == 0)
    rlex_ctx.token = ED_L_MUL;
  else if (strcmp(s, "^") == 0)
    rlex_ctx.token = ED_L_EXP;

  else if (strcmp(s, "==") == 0)
    rlex_ctx.token = ED_L_EQ;
  else if (strcmp(s, "!=") == 0)
    rlex_ctx.token = ED_L_NE;
  else if (strcmp(s, "/=") == 0)
    rlex_ctx.token = ED_L_NE;
  else if (strcmp(s, "rank") == 0)
    rlex_ctx.token = ED_L_RANK;

  else if (strcmp(s, "det") == 0)
    rlex_ctx.token = ED_L_DET;
  else if (strcmp(s, "inv") == 0)
    rlex_ctx.token = ED_L_INV;
  else if (strcmp(s, "kerbas") == 0)
    rlex_ctx.token = ED_L_KERBAS;
  else if (strcmp(s, "rr") == 0)
    rlex_ctx.token = ED_L_RR;
  else if (strcmp(s, "rech") == 0)
    rlex_ctx.token = ED_L_RECH;
  else if (strcmp(s, "rechr") == 0)
    rlex_ctx.token = ED_L_RECHR;
  else if (strcmp(s, "rrnz") == 0)
    rlex_ctx.token = ED_L_RRNZ;
  else if (strcmp(s, "rechnz") == 0)
    rlex_ctx.token = ED_L_RECHNZ;
  else if (strcmp(s, "transpose") == 0)
    rlex_ctx.token = ED_L_TRANSPOSE;

  else if (strcmp(s, "(") == 0)
    rlex_ctx.token = ED_L_LPAREN;
  else if (strcmp(s, ")") == 0)
    rlex_ctx.token = ED_L_RPAREN;
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_emit(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                    tstack<ed_matatom_t<element_type>> &rstack) {
  ...
  if ((msiform == ED_M_M_OP) || (msiform == ED_S_M_OP) ||
      (msiform == ED_I_M_OP)) {
    rstack.pop(A);
  } else {
    rstack.pop(B);
  ...
  case ED_L_EQ:
    C.int_val = A.mat_val == B.mat_val;
    break;
  case ED_L_NE:
    C.int_val = A.mat_val != B.mat_val;
    break;
  case ED_L_RANK:
    C.int_val = A.mat_val.get_rank_rr();
    break;
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_match(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                     int expected_token) {
  if (rlex_ctx.token == expected_token) {
    ed_matlexan<element_type>(rlex_ctx);
  } else {
    std::cerr << "Syntax error.\n";
    std::cerr << "Expected " << ed_mat_token_desc<element_type>(expected_token)
              << "; got " << ed_mat_token_desc<element_type>(rlex_ctx.token)
              << ".\n";
    exit(1);
  }
}
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_P(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack) {
  switch (rlex_ctx.token) {
  case ED_L_LPAREN:
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_U(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack) {
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_F(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack) {
  ed_mat_lex_ctx_t<element_type> save;
  switch (rlex_ctx.token) {
  case ED_L_PLUS:
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_T(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack) {
  ed_mat_lex_ctx_t<element_type> save;
...
// ----------------------------------------------------------------
template <class element_type>
static void ed_Q(ed_mat_lex_ctx_t<element_type> &rlex_ctx,
                 tstack<ed_matatom_t<element_type>> &rstack) {
  ed_mat_lex_ctx_t<element_type> save;
  ed_E<element_type>(rlex_ctx, rstack);
  while (1) {
...
template <class element_type>
void ed_cmd_line_mat_parse(int argc, char **argv, element_type zero,
                           element_type one) {
