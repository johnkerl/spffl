...
int tmvrat_f2_polymods_from_file(
  char *file_name, tvector<tmvrat<f2_polymod_t>> &rats, f2_poly_t m) {
  ...
  if (ifs.fail()) {
    std::cerr << "tmvrat_f2_polymods_from_file:  couldn't open \"" << file_name << "\"\n";
    return 0;
  }

  if (!tmvrat_f2_polymods_from_stream(ifs, rats, m)) {
    std::cerr << "tmvrat_f2_polymods_from_file:  scan failure reading \"" << file_name << "\"\n";
    ifs.close();
    return 0;
  }
  ifs.close();
