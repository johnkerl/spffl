...
// ----------------------------------------------------------------
static int num_irreds_deg_is(int p, int n);

static int num_irreds_deg_divides(int p, int n);

static void find_irreds(int p, int n, fp_poly_t *pirreds, int num_irreds);

static fppm_chart_t *alloc_fppm_chart(int num_rows, int num_cols);

static void free_fppm_chart(fppm_chart_t **ppchart);

static int fppm_chart_row_qcmp(const void *pv1, const void *pv2);

static void do_fppm_chart(fp_poly_t m, int p, int n, opts_t *popts);

static void fill_fppm_chart(fppm_chart_t *pchart, fp_poly_t m, int p, int n);

static void print_fppm_chart(fppm_chart_t *pchart, fp_poly_t m, int p, int n, opts_t *popts);
...
static void fill_fppm_chart(fppm_chart_t *pchart, fp_poly_t m, int p, int n) {
  ...
  qsort(pchart->rows, pchart->num_rows, sizeof(pchart->rows[0]), fppm_chart_row_qcmp);

  delete[] marks;
}

// ----------------------------------------------------------------
static void print_fppm_chart(fppm_chart_t *pchart, fp_poly_t m, int n, opts_t *popts) {
  int i, j;
