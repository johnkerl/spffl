...
// ----------------------------------------------------------------
static int num_irreds_deg_is(int n);

static int num_irreds_deg_divides(int n);

static void find_irreds(int n, f2_poly_t *pirreds, int num_irreds);

static f2_pm_chart_t *alloc_f2pm_chart(int num_rows, int num_cols);

static void free_f2pm_chart(f2_pm_chart_t **ppchart);

static int f2pm_chart_row_qcmp(const void *pv1, const void *pv2);

static void do_f2pm_chart(f2_poly_t m, int n, opts_t *popts);

static void fill_f2pm_chart(f2_pm_chart_t *pchart, f2_poly_t m, int n);

static void print_f2pm_chart(f2_pm_chart_t *pchart, f2_poly_t m, int n, opts_t *popts);
...
static void fill_f2pm_chart(f2_pm_chart_t *pchart, f2_poly_t m, int n) {
  ...
  qsort(pchart->rows, pchart->num_rows, sizeof(pchart->rows[0]), f2pm_chart_row_qcmp);
