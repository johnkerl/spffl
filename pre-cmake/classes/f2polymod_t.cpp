...
f2polymod_t f2polymod_t::operator/(f2polymod_t that)
{
	...
	if (!that.recip(bi)) {
		std::cerr
			<< "f2polymod_t::operator/:  zero or zero divisor: "
			<< that.residue
			<< " mod "
			<< that.modulus
			<< ".\n";
...
// ----------------------------------------------------------------
int f2polymod_t::operator==(int that) const
{
...
// ----------------------------------------------------------------
f2poly_t f2polymod_t::get_residue(void) const
{
	return this->residue;
}

// ----------------------------------------------------------------
f2poly_t f2polymod_t::get_modulus(void) const
{
	return this->modulus;
}
