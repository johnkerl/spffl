...
// ================================================================
template <class element_type> class teiginfo {
  // ================================================================
  private:
  struct eival_info_t {
  ...
  // ----------------------------------------------------------------
  teiginfo<element_type> &operator=(teiginfo<element_type> that) {
  ...
  friend std::ostream &operator<< <>(std::ostream &os, const teiginfo<element_type> &v);

  // ----------------------------------------------------------------
  int get_num_eivals(void) { return this->num_eivals; }
  ...
  void bounds_check(int which) {
    if ((which < 0) || (which >= this->num_eivals)) {
      std::cerr << "teiginfo:  index " << which << " out of bounds 0:" << this->num_eivals - 1
                << ".\n";
      exit(1);
    }
  }
...
static std::ostream &operator<<(std::ostream &os, const teiginfo<element_type> &finfo) {
