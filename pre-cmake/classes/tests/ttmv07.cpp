...
int main(int argc, char ** argv)
{
	...
	tmvpoly<f2polymod_t> F2_f = get_f(F2_zero, F2_one);
	...
	for (int argi = argb; argi < argc; argi++) {
		...
		f2polymod_t Fq_zero = f2polymod_t::prime_sfld_elt(0, m);
		f2polymod_t Fq_one  = f2polymod_t::prime_sfld_elt(1, m);
		...
		tmatrix<f2polymod_t> Fq_n = f2polymod_An_list(m, n);
		...
		tmatrix<f2polymod_t> Pn_Fq = f2polymod_Pn_list(m, n);
