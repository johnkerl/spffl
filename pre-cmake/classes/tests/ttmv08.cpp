...
// ----------------------------------------------------------------
tmvpoly<f2_polymod_t> get_f(f2_polymod_t &F2_zero, f2_polymod_t &F2_one, f2_poly_t &F2_m) {
  tmvpoly<f2_polymod_t> F2_f;
  if (!tmvpoly_f2_polymod_from_string(F2_f, "1.3.0+1.0.3+1.0.0", F2_m)) {
    std::cerr << "b&d!!\n";
    exit(1);
  }
...
#if 0
	tmvpoly<f2_polymod_t> F2_f =
		tmvpoly<f2_polymod_t>(F2_one, 0, 2) +
		tmvpoly<f2_polymod_t>(F2_one, 3, 0) +
		tmvpoly<f2_polymod_t>(F2_one, 1, 0) +
		tmvpoly<f2_polymod_t>(F2_one, 0, 0);
#endif
...
int main(int argc, char **argv) {
  ...
  tmvpoly<f2_polymod_t> F2_f = get_f(F2_zero, F2_one, F2_m);
