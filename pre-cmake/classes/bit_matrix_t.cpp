...
// ----------------------------------------------------------------
bit_matrix_t::bit_matrix_t(void)
{
	this->nullify();
}
	...
	bit_matrix_t & m)
{
	...
	while (1) {
		...
		if (iss.fail()) {
			...
			std::cerr << "bit_matrix_t scan failure: \""
				<< line << "\"\n";
			return is;
		}
	...
	if (min_cols != max_cols) {
		is.setstate(std::ios::badbit);
		std::cerr
			<< "bit_matrix_t >>:  ragged input.  # rows = "
			<< m.num_rows
			<< " min # cols = "
			<< min_cols
			<< " max # cols = "
...
bit_vector_t & bit_matrix_t::operator[](int row_index)
{
	if ((row_index < 0) || (row_index >= this->num_rows)) {
		std::cerr
			<< "bit_matrix_t array operator: row index "
			<< row_index
			<< " out of bounds "
			<< 0
			<< ":"
	...
	bit_matrix_t that)
{
	...
	||  (this->num_cols != that.num_cols)) {
		std::cerr
			<< "bit_matrix_t operator-():  Incompatibly sized "
			<< "arguments ("
...
// ----------------------------------------------------------------
int bit_matrix_t::get_kernel_basis(
	bit_matrix_t & rbas)
{
	int i, j;
	bit_matrix_t rr(*this);
	...
	int split_column)
{
	...
	rright = bit_matrix_t(this->num_rows,
...
// ----------------------------------------------------------------
int bit_matrix_t::get_num_rows(void)
{
	return this->num_rows;
}

// ----------------------------------------------------------------
int bit_matrix_t::get_num_cols(void)
{
