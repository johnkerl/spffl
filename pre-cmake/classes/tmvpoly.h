...
static std::ostream &operator<<(std::ostream &os, const tmvpoly<ktype> &poly);

template <typename ktype>
static std::ostream &operator<<(std::ostream &os, const tmvmonom<ktype> &poly);
...
template <typename ktype> class tmvmonom {
  ...
  tmvmonom(ktype c, int *arg_expts, int num_expts) {
    ...
    if (num_expts < 1) {
      std::cerr << "tmvmonom ctor:  need at least 1 variable; got " << num_expts << ".\n";
      exit(1);
    }
  ...
  // ----------------------------------------------------------------
  // 3.1.4
  friend std::ostream &operator<< <>(std::ostream &os, const tmvmonom &monom);
  ...
  void tex_print(std::ostream &os) { this->tex_or_pretty_print(os, 1); }
  void pretty_print(std::ostream &os) { this->tex_or_pretty_print(os, 0); }
  ...
  void check_varidx(int i) {
    if ((i < 0) || (i >= this->nvars)) {
      std::cerr << "varidx must be 0:" << this->nvars - 1 << "; got " << i << "\n";
      exit(1);
    }
  }
...
static std::ostream &operator<<(std::ostream &os, const tmvmonom<element_type> &monom) {
  os << monom.coeff;
...
template <typename ktype> class tmvpoly {
  ...
  void bounds_check_monomi(int monomi) {
    if ((monomi < 0) || (monomi >= this->nmonoms)) {
      std::cerr << "tmvpoly:  monomial index " << monomi << " out of bounds 0:" << this->nmonoms - 1
                << ".\n";
      exit(1);
    }
  }
  ...
  // ----------------------------------------------------------------
  int get_num_monoms(void) { return this->nmonoms; }

  // ----------------------------------------------------------------
  int get_nvars(void) {
    if (this->nmonoms == 0) {
      std::cerr << "tmvpoly::get_nvars:  doesn't apply to zero polynomial.\n";
      exit(1);
    }
  ...
  friend std::ostream &operator<< <>(std::ostream &os, const tmvpoly &poly);
...
// ----------------------------------------------------------------
template <class element_type>
static std::ostream &operator<<(std::ostream &os, const tmvpoly<element_type> &poly) {
  if (poly.nmonoms == 0) {
    os << "0";
  } else {
