...
static tmvpoly<f2_polymod_t> F2_Fq_embed(
  tmvpoly<f2_polymod_t> &f, f2_polymod_t &Fq_zero, f2_polymod_t &Fq_one);

// xxx move to fp_polymod_convert.cpp
static tmvpoly<fp_polymod_t> Fp_Fq_embed(tmvpoly<fp_polymod_t> &f, fp_poly_t &m);

static tvector<f2_polymod_t> F2_frob(tvector<f2_polymod_t> &v);

static tmatrix<f2_polymod_t> F2_frobit(tvector<f2_polymod_t> &v, int n);
...
int f2aplist_main(int argc, char **argv) {
  ...
  for (argi = argfb, i = 0; i < num_F; i++, argi++) {
    if (!tmvpoly_f2_polymod_from_string(arg_f[i], argv[argi], F2_m)) {
  ...
  for (i = 1; i < num_F; i++) {
    if (arg_f[i].get_nvars() != nv) {
      std::cerr << argv[0] << ": nvars must be the same for all functions.\n";
  ...
  for (argi = argdb; argi < argc; argi++) {
    ...
    if (homogenize)
      space = f2_polymod_Pn_list(Fq_m, nv - 1);
    else
      space = f2_polymod_An_list(Fq_m, nv);
    ...
    for (j = 0; j < oS; j++) {
      ...
      for (i = 0; i < point_degree; i++) {
        ...
        if (!space.find_row(frobit[i], markidx)) {
          std::cout << "Coding error detected: file " << __FILE__ << " line " << __LINE__ << ".\n";
          exit(1);
        }
        marks[markidx] = 1;
      }
...
int f2apolist_main(int argc, char **argv) {
  ...
  for (int argi = argb; argi < argc; argi++) {
    ...
    tmatrix<f2_polymod_t> An_Fq = f2_polymod_An_list(Fq_m, nv);
    ...
    tmatrix<f2_polymod_t> Pn_Fq = f2_polymod_Pn_list(Fq_m, nv);
...
static void f2apsing_usage(char *argv0) {
  std::cerr << "Usage: " << argv0 << " [-v] [-v] {f} {extension degrees ... }\n";
  exit(1);
}
...
int f2apsing_main(int argc, char **argv) {
  ...
  for (int argi = argb; argi < argc; argi++) {
    ...
    delete[] funcs;
    std::cout << "f " << (num_singular_f ? "" : "non") << "singular (" << num_singular_f << " / "
              << An_Fq.get_num_rows() << ")\n";
    std::cout << "F " << (num_singular_F ? "" : "non") << "singular (" << num_singular_F << " / "
              << Pn_Fq.get_num_rows() << ")\n";
  }
...
static void f2apjac_usage(char *argv0) {
  std::cerr << "Usage: " << argv0 << " [-h] {f} {extension degree} {f file} {points file}\n";
...
int fpaplist_main(int argc, char **argv) {
  ...
  for (int argi = argb; argi < argc; argi++) {
    ...
    tmatrix<fp_polymod_t> An_Fq = fp_polymod_An_list(Fq_m, nv);
    int qn = An_Fq.get_num_rows();
    ...
    tmatrix<fp_polymod_t> Pn_Fq = fp_polymod_Pn_list(Fq_m, nv);
    int oP = Pn_Fq.get_num_rows();
...
static void fpapsing_usage(char *argv0) {
  std::cerr << "Usage: " << argv0 << " [-v] [-v] {p} {f} {extension degrees ... }\n";
  exit(1);
}
