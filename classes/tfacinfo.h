...
public:
	...
	tfacinfo(void)
	{
		...
		this->pfactors_and_counts =
			new factor_and_count_t[ this->num_allocated];
		this->have_unit           = 0;
	...
	// ----------------------------------------------------------------
	tfacinfo<element_type> &
	operator=(tfacinfo<element_type> that)
	{
...
public:
		...
		int count)
	{
		...
		if (this->num_distinct >= this->num_allocated) {
			...
			this->pfactors_and_counts = new factor_and_count_t[
				this->num_allocated];
			for (int i = 0; i < this->num_distinct; i++)
	...
	// ----------------------------------------------------------------
	void merge(
		tfacinfo<element_type> & that)
	{
		for (int i = 0; i < that.num_distinct; i++)
			this->insert_factor(
				that.pfactors_and_counts[i].factor,
	...
	tvector<element_type> get_all_divisors(
		element_type one)
	{
		if (this->num_distinct <= 0) {
			if (!this->have_unit) {
	...
	// ----------------------------------------------------------------
	// The output will be sorted from smallest to largest.
	int get_maximal_proper_divisors(
		tvector<element_type> & rv,
		element_type one)
	{
		if (this->num_distinct <= 0) {
			if (!this->have_unit) {
