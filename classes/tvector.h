...
public:
	...
	tvector(int init_num_elements)
	{
		if (init_num_elements <= 0) {
			std::cerr
				<< "tvector::tvector():  Vector size must be > 0; got "
				<< init_num_elements
				<< ".  Exiting."
				<< std::endl;
			exit(1);
	...
	operator=(tvector<element_type> that)
	{
		this->num_elements = that.num_elements;
		if (this->elements == 0) {
			this->elements = new element_type[that.num_elements];
	...
	// The istream input operator expects only whitespace-delimited elements, e.g.
	// "1 2 3".  The bracket_in() method, by contrast, expects whitespace-delimited
	// elements, surrounded by brackets:  e.g. "[1 2 3]".
	...
	int load_from_file(char * file_name)
	{
		...
		if (ifs.fail()) {
			std::cerr << "tvector::load_from_file:  couldn't open \""
				<< file_name << "\"\n";
			return 0;
		}
		...
		tmatrix<element_type> A)
	{
		...
		if (this->num_elements != Anr) {
			std::cerr
				<< "tvector operator*(): Incompatibly dimensioned "
				<< "operands (" << this->num_elements << ","
				<< Anr << "x" << Anc << ")." << std::endl;
			exit(1);
		}
	...
	// ----------------------------------------------------------------
	// Instantiate this template only for data types for which a gcd() function has
	// been implemented.
	...
	void check_equal_lengths(tvector<element_type> & that)
	{
		if (this->num_elements != that.num_elements) {
			std::cerr
				<< "tvector operator+():  Incompatibly sized arguments ("
				<< this->num_elements << ", "
	...
	void trim_num_elements(int new_num_elements)
	{
		if ((new_num_elements < 1) || (new_num_elements > this->num_elements)) {
			std::cerr << "tvector trim_num_elements: new count "
				<< new_num_elements << " out of bounds "
				<< 1 << ":" << this->num_elements << std::endl;
			exit(1);
		}
	...
	tvector<element_type> & v)
{
	...
	if (!v.elements || (v.num_elements < 1)) {
		...
		std::cerr <<
			"tvector string >>:  must already have one element.\n";
		return iss;
	}
	...
	while (1) {
		...
		if (iss.fail()) {
			std::cerr
				<< "tvector istringstream >>: scan failure"
