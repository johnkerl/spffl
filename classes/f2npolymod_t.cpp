...
// ----------------------------------------------------------------
int f2npolymod_t::get_char(void)
{
	return 2;
}
...
f2npolymod_t f2npolymod_t::operator/(f2npolymod_t that)
{
	...
	if (!that.recip(bi)) {
		std::cerr
			<< "f2npolymod_t::operator/:  zero or zero divisor: "
			<< that.residue
			<< " mod "
			<< that.modulus
			<< ".\n";
...
int f2npolymod_t::recip(f2npolymod_t & rinv)
{
	...
	if (!g.get_coeff(0).recip(c0i)) {
		std::cerr
			<< "f2npolymod_t::recip:  zero or zero divisor "
			<< " in GCD "
			<< g
			<< ".\n";
		exit(1);
...
// ----------------------------------------------------------------
int f2npolymod_t::operator==(int that) const
{
...
// ----------------------------------------------------------------
f2npoly_t f2npolymod_t::get_residue(void) const
{
	return this->residue;
}

// ----------------------------------------------------------------
f2npoly_t f2npolymod_t::get_modulus(void) const
{
	return this->modulus;
}
